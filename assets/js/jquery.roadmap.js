	...
	$.fn.roadmap = function(events, opts) {
		...
		var defaults = {
			...
			eventTemplate: '<div class="event">' +
							...
							'</div>',
			eventTemplate2: '<div class="event">' +
								'<div class="event__content">####CONTENT###</div>' +
								'<div class="event__date">####DATE###</div>' +
							'</div>'
		};
		...
		var buildEvent = function(event, key) {
			var tmplates = (key%2!=0)?settings.eventTemplate2:settings.eventTemplate;
			var html = '<li class="' + settings.rootClass + '__events__event">' + tmplates + '</li>';
			if(event.date!='' && event.content!=""){
				html = html.replace('####DATE###', event.date);
				html = html.replace('####CONTENT###', event.content);
			}else{
				html = '<div class="event"></div>';
			}
